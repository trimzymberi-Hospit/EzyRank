...
export default function TopViewedPagesCard({data, tittle, loading, description}) {
  ...
  return (
    <div className="flex  md:w-1/2 w-full flex-col gap-6 rounded-2xl bg-app-third p-6 text-text-secondary min-h-[510px] max-h-[510px] overflow-hidden">
      ...
      <div className='flex items-center w-full justify-between'>
        <h1 className="text-2xl text-white font-bold font-logo">
          Top Visited Pages
        </h1>
        <div className="relative flex justify-end items-end">
              ...
              {isOpen && (
                <p className="absolute text-gray-300 top-8 right-0 w-60 bg-blue-900 text-xs p-3 rounded-xl shadow-lg z-20">
                  {description}
                </p>
              )}
            </div>
      ...
      <div className="flex justify-between">
        <h4 className="text-xl text-white">Page path</h4>
      </div>
      ...
      <div className="flex flex-col gap-3 overflow-y-auto pr-2 scrollbar-dark">
        {data.length > 0 && (
          data.map((c, idx) => {
            const pagePath = c?.pagePath || "Unavailable";
            ...
            return (
              ...
              >
                <h4 className="text-sm break-all">{pagePath}</h4>
              </div>
            );
          })
        )}
