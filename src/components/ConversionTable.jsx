              ...
              data.map((conversion, index) => (
                <tr key={index} className="bg-neutral-primary">
                  <th className="px-6 py-4 font-medium text-heading whitespace-nowrap">
                    {conversion?.dl_description.startsWith(" -") ? conversion?.dl_description.replace("-", "").trim() : conversion?.dl_description === "(not set)" || conversion?.dl_description === ""  ? "Booking" : conversion?.dl_description}
                  </th>
